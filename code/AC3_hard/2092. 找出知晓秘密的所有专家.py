# -*- coding: utf-8 -*-
# @Time    : 2021/12/22 16:19
# @Github  : https://github.com/monijuan
# @CSDN    : https://blog.csdn.net/qq_34451909
# @File    : 2092. 找出知晓秘密的所有专家.py
# @Software: PyCharm
# ===================================
...
class Solution:
    ...
    def findAllPeople(self, n: int, meetings: List[List[int]], firstPerson: int) -> List[int]:
        meetdict = defaultdict(list)
        for a,b,time in meetings:
            meetdict[a].append((b,time))
            meetdict[b].append((a,time))
        queue = [(0,0),(firstPerson,0)] # id,time
        mintime = [float('inf')]*n
        mintime[0]=mintime[firstPerson]=0
        while queue:
            nowid,time_know = heapq.heappop(queue)
            for nextid,time_meet in meetdict[nowid]:
                if time_meet>=time_know and time_meet<mintime[nextid]:
                    heapq.heappush(queue,(nextid,time_meet))
                    mintime[nextid] = time_meet
        return  [id for id in range(n) if mintime[id]!=float('inf')]
